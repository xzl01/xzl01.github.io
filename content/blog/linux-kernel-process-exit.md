...
1. 进行一个系统调用：`do_exit`
2. 如果销毁的进程有子进程的存在，则让子进程的father置为1（init进程）如果该子进程已经处于僵死(ZOMBIE)状态，则向进程1发送子进程中止信号SIGCHLD。
   ```c
...
3. 关闭进程打开的所有文件，对当前目录和i节点同步操作
4. 如果当前进程是会话头领(leader)进程并且其有控制终端，则释放该终端。
5. 把当前进程置为僵死状态，表明当前进程已经释放了资源。并保存将由父进程读取的退出码

然后父进程在运行子进程的时候，一般都会运行`wait()`函数和`waitpid()`函数：父进程等待某个子进程终止时返回的信号，收到信号后由父进程终止僵死状态的子进程：
...
1. &#x20;父进程把子进程的运行时间累加到自己的进程变量中
2. 释放对应子进程的进程描述结构体，置空
